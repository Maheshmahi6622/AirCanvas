...
model_stats = {
    "MediaPipe": {"frames": 0, "detections": 0, "time": 0},
    "YOLOv8": {"frames": 0, "detections": 0, "time": 0},
    "TF-HandPose": {"frames": 0, "detections": 0, "time": 0},
}
try:
    yolo_model = YOLO("yolo11n.pt")  # Use hand-trained model path
except:
    yolo_model = None

try:
    from tf_handpose import HandPose
    tf_model = HandPose()
except:
    tf_model = None
def speak_thread():
...
def plot_hand_model_comparison():
    methods = []
    fps_vals = []
    acc_vals = []
    ...
    for model, stats in model_stats.items():
        ...
        fps_vals.append(stats["frames"] / stats["time"])
        acc_vals.append(100 * stats["detections"] / stats["frames"])
    ...
    ax[0].bar(x, fps_vals, width, color='skyblue')
    ax[0].set_title("FPS Comparison")
    ax[0].set_ylabel("Frames per Second")
    ax[0].set_xticks(x)
    ax[0].set_xticklabels(methods)

    ax[1].bar(x, acc_vals, width, color='lightgreen')
    ax[1].set_title("Detection Accuracy")
    ax[1].set_ylabel("Detection Rate (%)")
    ax[1].set_xticks(x)
    ax[1].set_xticklabels(methods)

    plt.suptitle("Hand Tracking Model Comparison")
    plt.tight_layout()
    plt.show()
...
while ret:
    ...
    media_detected = bool(result.multi_hand_landmarks)
    model_stats["MediaPipe"]["frames"] += 1
    model_stats["MediaPipe"]["detections"] += int(media_detected)
    model_stats["MediaPipe"]["time"] += (time.time() - start_time)

# YOLOv8 detection
    if yolo_model:
        start = time.time()
        yolo_result = yolo_model.predict(frame, verbose=False)[0]
        model_stats["YOLOv8"]["frames"] += 1
        model_stats["YOLOv8"]["detections"] += int(len(yolo_result.boxes) > 0)
        model_stats["YOLOv8"]["time"] += (time.time() - start)

    # TF Handpose detection
    if tf_model:
        start = time.time()
        tf_result = tf_model.estimate_hands(frame)
        model_stats["TF-HandPose"]["frames"] += 1
        model_stats["TF-HandPose"]["detections"] += int(len(tf_result) > 0)
        model_stats["TF-HandPose"]["time"] += (time.time() - start)
